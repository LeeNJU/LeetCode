...
//解法描述：动态规划，s[i] == '0'的时候,不能被单独decode，必须前面是1或者2才能被decode.所以是dp[i + 2] = dp[i]
//         如果s[i] != '0',可以被单独decode,dp[i + 2] = dp[i + 1], 这时还要看当前元素是不是能和前一个元素构成合理
//         的decode方式,所以还要再加上dp[i + 2] += dp[i]
